...
AbstractEvaluator::Task EvaluatorChi2Contribution::makeTask(const FrameDescriptor &frame) const noexcept
{
	...
	return t.then([this](Task task){
		...
		double dist=res->get();
		double delta=(dist-_dist.distance())/_dist.err(dist);
		double chi2c=delta*delta;
		auto result=std::make_shared<CalcResult<double>>(chi2c);
