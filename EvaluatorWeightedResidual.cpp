...
AbstractEvaluator::Task EvaluatorWeightedResidual::makeTask(const FrameDescriptor &frame) const noexcept
{
	...
	return t.then([this](Task task){
		...
		double dist=res->get();
		double wres=(dist-_dist.distance())/_dist.err(dist);
		auto result=std::make_shared<CalcResult<double>>(wres);
