...
QVariant EvaluatorsTreeModel::data(const QModelIndex &index, int role) const
{
	...
	if (role == Qt::DisplayRole || role == Qt::EditRole) {
		...
		if (item.isEvaluatorsClass()) { // index is evaluator
			if (index.column() == 0) {
				return QString::fromStdString(
					evalName(item.classRow(), index.row()));
				...
				return QVariant::fromValue<ButtonFlags>(
					btnFlags);
			}
			return QVariant();
		}
...
bool EvaluatorsTreeModel::setData(const QModelIndex &index,
	...
	if (index.data(Qt::EditRole).type() == QVariant::StringList) {
		...
		if (oldData.userType() == evalType) {
			...
			if (value.type() == QVariant::Int) {
				simName =
					_simTypes[value.toInt()].toStdString();
			} else if (value.type() == QVariant::String) {
			...
			QList<EvalId> ptrs;
			int cr = classRow(
				oldData.value<QList<EvalId>>().front());
			if (value.userType()
			    == QVariant::fromValue(QList<int>()).userType()) {
		...
		pendingEvals[item.evaluatorRow()]->setSetting(index.row(),
							      data);
	} else {
		pendingEvals[item.evaluatorRow()]->setSetting(index.row(),
							      value);
...
QVariant EvaluatorsTreeModel::headerData(int section,
...
QModelIndex EvaluatorsTreeModel::index(int row, int column,
				       const QModelIndex &parentIndex) const
{
	if (!hasIndex(row, column, parentIndex)) {
		return QModelIndex();
	}
	// Item holds an info about the PARENT of the index, not the index
	// itslef

	// return class
	if (!parentIndex.isValid()) {
		return createIndex(row, column,
				   EvaluatorsTreeItem::invalidItem().intptr);
	}

	auto parentItem =
		EvaluatorsTreeItem::fromIntptr(parentIndex.internalId());
	// return evaluator
	if (parentItem.isInvalid()) { // the parent is a class
		auto item =
			EvaluatorsTreeItem::evaluatorsClass(parentIndex.row());
		return createIndex(row, column, item.intptr);
	}
	// return property
	if (parentItem.isEvaluatorsClass()) {
		EvaluatorsTreeItem item(parentItem.classRow(),
					parentIndex.row());
		return createIndex(row, column, item.intptr);
	}
	return QModelIndex();
...
/*
EvaluatorsTreeModel::EvalPtr EvaluatorsTreeModel::eval(const
EvaluatorsTreeModel::EvalPtr &oldEval, int newEvalRow)
{
...
EvaluatorsTreeModel::MutableEvalPtr
EvaluatorsTreeModel::removeEvaluator(int evRow)
{
...
void EvaluatorsTreeModel::setEvaluatorName(const QModelIndex &index,
...
void EvaluatorsTreeModel::setEvaluatorOption(const QModelIndex &index,
...
QModelIndex EvaluatorsTreeModel::duplicateEvaluator(const QModelIndex &index)
{
	...
	if (item.isEvaluatorsClass()) {
		...
		int origEvalType = types.indexOf(
			QString::fromStdString(origEval.className()));
...
QVariantMap EvaluatorsTreeModel::evaluatorsFromLegacy(QTextStream &in) const
{
	...
	if (lines[0] == "Rmp" || lines[0] == "RDAMean"
	    || lines[0] == "RDAMeanE") {
		QVariantMap propMap;
		...
		for (const QString &line : lines) {
			...
			const QString &evName =
				QString::fromStdString(eval.name());
			if (!evName.isEmpty()) {
				propMap[evName] = _storage.propMap(eval);
			}
		}
		const QString &className = QString::fromStdString(
			EvaluatorDistance(_storage, "").className());
		evals[className] = propMap;
	} else { // Labelling Postions
		QString pdb = QFileDialog::getOpenFileName(
			0,
		...
		for (const QString &line : lines) {
			...
			auto eval = EvaluatorPositionSimulation(_storage, line,
								pdb);
			if (eval.simulationType()
			    == Position::SimulationType::NONE) {
				std::cerr << "Could not parse position "
						     + eval.name()
						     + ", skipping\n"
					  << std::flush;
				continue;
		...
		const QString &className = QString::fromStdString(
			EvaluatorPositionSimulation(_storage, "").className());
		evals[className] = propMap;
	}
	return evals;
}
...
QVariantMap EvaluatorsTreeModel::evaluators() const
{
	...
	for (const auto &pair : _storage.evals()) {
		...
		const QString &className =
			QString::fromStdString(eval.className());
...
QString EvaluatorsTreeModel::uniqueEvalName(const QVariantMap &evalMap,
...
const AbstractEvaluator &EvaluatorsTreeModel::eval(int classRow,
						   int evalRow) const
{
	if (classRow > 0) {
...
const AbstractEvaluator &
EvaluatorsTreeModel::eval(const EvaluatorsTreeItem &item) const
{
...
QList<int> EvaluatorsTreeModel::evalRowList(const QList<EvalId> &selected) const
{
	QList<int> list;
...
QVariantMap EvaluatorsTreeModel::propMap(const AbstractEvaluator &eval) const
{
	...
	for(int p=0; p<eval.settingsCount(); ++p) {
		...
		if(opt.second.userType()==evalType) {
			...
			const auto&
type=opt.second.value<Position::SimulationType>();
