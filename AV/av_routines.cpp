...
int calculate1R(double L, double W, double R, int atom_i, double dg,
		...
		double linkersphere, int linknodes, unsigned char *density) // returns density array
{
	// grid
...
calculate1R(double L, double W, double R, int atom_i,
	    double dg, double vdWRMax, double linkersphere,
	    int linknodes, const std::vector<Eigen::Vector4f> &xyzW)
{
	int NAtoms=xyzW.size();
	std::vector<double> xLocal, yLocal, zLocal,vdWR;
	xLocal.reserve(NAtoms);
	yLocal.reserve(NAtoms);
	zLocal.reserve(NAtoms);
	vdWR.reserve(NAtoms);
	for(int i=0; i<NAtoms; ++i)
	{
		xLocal.push_back(xyzW[i][0]);
		yLocal.push_back(xyzW[i][1]);
		zLocal.push_back(xyzW[i][2]);
		vdWR.push_back(xyzW[i][3]);

	}
	int npm = (int)floor(L / dg);
	int ng = 2 * npm + 1, n;
	int ng3 = ng * ng * ng;
	std::vector<unsigned char> density(ng3,0);
	int nResult=calculate1R(L,W,R,atom_i,dg,
				xLocal.data(),yLocal.data(),zLocal.data(),
				vdWR.data(),NAtoms,vdWRMax,
				linkersphere,linknodes,density.data());
	float x0 = xyzW[atom_i][0], y0 = xyzW[atom_i][1], z0 = xyzW[atom_i][2];
	std::vector<Eigen::Vector3f> points;
	points.reserve(nResult);
	n = 0; int dn = 0;
	int ng2=ng*ng, offset;
	for (int ix = -npm; ix <= npm; ix++)
	{
		offset = ng2 * (ix + npm) + npm;
		for (int iy = -npm; iy <= npm; iy++)
