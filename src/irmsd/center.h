					 ...
					 const int n_frames, const int n_atoms)
{
	/* Center a trajectory containing multiple conformations inplace.
	   ...
	   Also compute the traces of the centered conformations, which are
	   necessary for RMSD.
	*/
	int i, k;
	float *confp;
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(coords, traces) private(         \
	sx_, sy_, sz_, trace_, mux_, muy_, muz_, sxf, syf, szf, confp, i, x,   \
	y, z, x2, y2, z2, sx, sy, sz, trace)
#endif
	for (k = 0; k < n_frames; k++) {
		...
		for (i = 0; i < n_atoms / 4; i++) {
			...
			sx_ = _mm_add_pd(sx_,
					 _mm_cvtps_pd(_mm_movehl_ps(x, x)));
			sy_ = _mm_add_pd(sy_,
					 _mm_cvtps_pd(_mm_movehl_ps(y, y)));
			sz_ = _mm_add_pd(sz_,
					 _mm_cvtps_pd(_mm_movehl_ps(z, z)));
			confp += 12;
		}
		...
		/* Put everything into the first value. We're doing this here,
		 * as */
		...
		for (i = 0; i < n_atoms / 4; i++) {
			...
			trace_ = _mm_add_pd(
				trace_, _mm_cvtps_pd(_mm_movehl_ps(x2, x2)));
			trace_ = _mm_add_pd(
				trace_, _mm_cvtps_pd(_mm_movehl_ps(y2, y2)));
			trace_ = _mm_add_pd(
				trace_, _mm_cvtps_pd(_mm_movehl_ps(z2, z2)));
