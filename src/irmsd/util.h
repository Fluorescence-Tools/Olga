...
static INLINE void aos_interleaved_storeu(float *p, __m128 x, __m128 y,
					  __m128 z)
{
	...
	p points to memory that is supposed to be layed out in atom major order,
	like
	...
	And x, y, and z are SSE float4s that contain the x, y and z coordinates
	of four atoms. In order to actually store the 12 floats in memory, you
	need to do some shuffles first.
...
static INLINE void aos_deinterleaved_load(const float *S, __m128 *x, __m128 *y,
					  __m128 *z)
{
...
static INLINE void aos_deinterleaved_loadu(const float *S, __m128 *x, __m128 *y,
					   __m128 *z)
{
	/* Deinterleaved load of x, y, z coordinates of 4 atoms in atom-major
