...
int quartic_equation_solve_exact(double *r1, double *r2, double *r3, double *r4,
				 int *nr12, int *nr34, double d0, double d1,
...
int solve_cubic_equation(double c3, double c2, double c1, double c0, double *x1,
			 double *x2, double *x3)
{
	double s1, s2;
	double a2 = c2 / c3;
...
float DirectSolve(float lambda, float C_0, float C_1, float C_2)
{
	...
	double r1, r2, r3, r4;
	int nr1, nr2;
	quartic_equation_solve_exact(&r1, &r2, &r3, &r4, &nr1, &nr2,
				     (double)C_0, (double)C_1, (double)C_2, 0.0,
				     1.0);
	result = max(r1, r2);
...
float NewtonSolve(float lambda, float C_0, float C_1, float C_2)
{
	...
	for (i = 0; i < maxits; i++) {
		...
		lambda = lambda_old
			 - (a * lambda_old + C_0)
				   / (2.0f * lambda2 * lambda_old + b + a);
		if (fabsf(lambda - lambda_old) < fabsf(tolerance * lambda))
			break;
	}
	if (fabsf(lambda - lambda_old) >= fabsf(100 * tolerance * lambda)) {
		printf("RMSD Warning: No convergence after %d iterations: Lambda,Lambda0,Diff,Allowed = %f, %f, %f, %f \n",
		       maxits, lambda, lambda_old, fabsf(lambda - lambda_old),
		       fabsf(tolerance * lambda));
	}
...
{
	/* Compute the RMSD and (optionally) the rotation matrix from M and G.
	Core routine of the theobald QCP method.
	...
	*/
	...
	float k2233_2323, k1233_1323, k1223_1322, k0223_0322, k0233_0323,
		k0213_0312;
	float q0, q1, q2, q3, qsqr, normq, a2, x2, y2, z2, xy, az, zx, ay, yz,
		ax;
	...
	detK = k01 * k01 * k23 * k23 - k22 * k33 * k01 * k01
	...
	if (computeRot != 0) {
		...
		if (qsqr < 1e-11f) {
			fprintf(stderr,
				"%s UNCONVERGED ROTATION MATRIX. RETURNING IDENTITY=%d\n",
				__FILE__, __LINE__);
			rot[0] = rot[4] = rot[8] = 1.0;
			rot[1] = rot[2] = rot[3] = rot[5] = rot[6] = rot[7] =
				0.0;
		} else {
			normq = sqrt(qsqr);
			q0 /= normq;
...
float msd_axis_major(const int nrealatoms, const int npaddedatoms,
		     const int rowstride, const float *aT, const float *bT,
		     const float G_a, const float G_b)
{
	/*   Computes the mean-square-deviation between two centered structures
	 * in axis-major format.
	 *
	 *   Structure setup for this function:
	 *
	 *   structures are stored axis major, and if this file is compiled with
	 *   -DALIGNED, possibly with extra padding to ensure you meet two
	 * constraints:
	 *       - the number of elements in a row must be a multiple of 4
	 *       - the first element in each row must be aligned to a 16 byte
	 * boundary
	 *
	 *   note that if you meet the second condition for the first row, and
	 * meet the first condition, the alignment will automatically be
	 * satisfied for every row.
	 *
	 ...
	 *
	 *   if your structure has a number of atoms that is not a multiple of
	 * 4, you must pad it out to a multiple of 4 using zeros (using anything
	 * other than zero will make the calculation go wrong).
	 *
	 ...
	 *
	 *       npaddedatoms: the number of atoms in the structure including
	 * padding atoms; should equal nrealatoms rounded up to the next
	 * multiple of 4 THIS IS ONLY USED IF COMPILED WITH -DALIGNED. OTHERWISE
	 *                     IT IS IGNORED.
	 *
	 *       rowstride:    the offset in elements between rows in the
	 * arrays. will prob be equal to npaddedatoms, but you might use
	 * something else if (for example) you were subsetting the structure
	 *
	 *       aT:           pointer to start of first structure (A). should
	 * be aligned to a 16-byte boundary if compiled with -DALIGNED
	 *
	 *       bT:           pointer to start of second structure (B). should
	 * be aligned to a 16-byte boundary if compiled with -DALIGNED
	 *
	 *       G_a:          trace of A'A
	 *
	...
	for (k = 0; k < niters; k++) {
		...
		if (k == niters - 1) {
			ax = _mm_set_ps(
				mask[0] ? aTx[0] : 0, mask[1] ? aTx[1] : 0,
				mask[2] ? aTx[2] : 0, mask[3] ? aTx[3] : 0);
			ay = _mm_set_ps(
				mask[0] ? aTy[0] : 0, mask[1] ? aTy[1] : 0,
				mask[2] ? aTy[2] : 0, mask[3] ? aTy[3] : 0);
			az = _mm_set_ps(
				mask[0] ? aTz[0] : 0, mask[1] ? aTz[1] : 0,
				mask[2] ? aTz[2] : 0, mask[3] ? aTz[3] : 0);
			b = _mm_set_ps(
				mask[0] ? bTx[0] : 0, mask[1] ? bTx[1] : 0,
				mask[2] ? bTx[2] : 0, mask[3] ? bTx[3] : 0);
		} else {
			ax = _mm_loadu_ps(aTx);
			ay = _mm_loadu_ps(aTy);
			az = _mm_loadu_ps(aTz);
		...
		if (k == niters - 1) {
			b = _mm_set_ps(
				mask[0] ? bTy[0] : 0, mask[1] ? bTy[1] : 0,
				mask[2] ? bTy[2] : 0, mask[3] ? bTy[3] : 0);
		} else {
			b = _mm_loadu_ps(bTy);
		}
#endif
		...
		if (k == niters - 1) {
			b = _mm_set_ps(
				mask[0] ? bTz[0] : 0, mask[1] ? bTz[1] : 0,
				mask[2] ? bTz[2] : 0, mask[3] ? bTz[3] : 0);
		} else {
			b = _mm_loadu_ps(bTz);
		}
#endif
...
float msd_atom_major(const int nrealatoms, const int npaddedatoms,
		     const float *a, const float *b, const float G_a,
		     const float G_b, int computeRot, float rot[9])
{
	 ...
	 *       - if the number of atoms is not divisible by four, the
	 * structure is padded out with dummy atoms with zero in each coordinate
	 * up to an even multiple of 4 atoms.
	 ...
	 *       npaddedatoms: the number of atoms in the structure including
	 * padding atoms; should equal nrealatoms rounded up to the next
	 * multiple of 4. THIS IS ONLY USED IF COMPILED WITH -DALIGNED
	 *
	 *       a:            pointer to start of first structure (A). should
	 * be aligned to a 16-byte boundary if compiled with -DALIGNED
	 *
	 *       b:            pointer to start of second structure (B). should
	 * be aligned to a 16-byte boundary if compiled with -DALIGNED
	 *
	 *       G_a:          trace of A'A
	 *
	 *       G_b:          trace of B'B
	 *
	 *       computeRot:   if 0, the rotation matrix will not be computed.
	 * Otherwise, it will be computed and stored in rot
	 *
	 *       rot:          output variable where the rotation matrix will be
	 * stored, if computeRot != 0.
	 */
	int niters, k;
#ifndef ALIGNED
	static const int masks[4][4] = {
	...
	for (k = 0; k < niters; k++) {
		...
		if (k == niters - 1) {
			...
			/* 6  7  8  */
			/* 9  10 11 */
			ax = _mm_set_ps(mask[0] ? a[0] : 0, mask[1] ? a[3] : 0,
					mask[2] ? a[6] : 0, mask[3] ? a[9] : 0);
			ay = _mm_set_ps(mask[0] ? a[1] : 0, mask[1] ? a[4] : 0,
					mask[2] ? a[7] : 0,
					mask[3] ? a[10] : 0);
			az = _mm_set_ps(mask[0] ? a[2] : 0, mask[1] ? a[5] : 0,
					mask[2] ? a[8] : 0,
					mask[3] ? a[11] : 0);

			bx = _mm_set_ps(mask[0] ? b[0] : 0, mask[1] ? b[3] : 0,
					mask[2] ? b[6] : 0, mask[3] ? b[9] : 0);
			by = _mm_set_ps(mask[0] ? b[1] : 0, mask[1] ? b[4] : 0,
					mask[2] ? b[7] : 0,
					mask[3] ? b[10] : 0);
			bz = _mm_set_ps(mask[0] ? b[2] : 0, mask[1] ? b[5] : 0,
					mask[2] ? b[8] : 0,
					mask[3] ? b[11] : 0);
		} else {
