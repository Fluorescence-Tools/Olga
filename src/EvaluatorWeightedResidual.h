...
private:
	...
	std::string _name;
public:
	EvaluatorWeightedResidual(const TaskStorage& storage,const std::string& name);
	...
	Setting setting(int row) const
	{
		...
		EvalId id=_storage.isValid(_distCalc)?_distCalc:_storage.evaluatorDistance;
	...
	virtual void setSetting(int row, const QVariant& val)
	{
		...
		auto eval=static_cast<const EvaluatorDistance&>(_storage.eval(_distCalc));
		_dist=eval._dist;
	}
