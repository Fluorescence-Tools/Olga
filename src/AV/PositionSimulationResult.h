...
public:
	              ...
	              float maxR) const
	{
		...
		for (const Eigen::Vector4f &point : _points) {
			...
			float
			w=(std::exp(-rMpSq/(2.0f*10.0f*10.0f))+1.0f)*0.5f;*/
			totalVol += point[3];
...
protected:
	...
	// A layer of empty cells around the grid, which is added to avoid
	// boundary-check so in the end size of the grid is a bit bigger, then
