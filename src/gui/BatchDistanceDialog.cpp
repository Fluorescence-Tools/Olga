...
BatchDistanceDialog::BatchDistanceDialog(QWidget *parent,
					 EvaluatorsTreeModel &evModel)
    : QDialog(parent), _evModel(evModel), ui(new Ui::BatchDistanceDialog)
{
	ui->setupUi(this);
...
void BatchDistanceDialog::accept()
{
	...
	for (int i = 0; i < ui->lpList1->count(); ++i) {
		...
		for (int j = i + 1; j < ui->lpList2->count(); ++j) {
			...
			QModelIndex newDist =
				_evModel.duplicateEvaluator(rootEval);
