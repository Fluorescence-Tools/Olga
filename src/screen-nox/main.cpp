...
int main(int argc, char *argv[])
{
	...
	parser.addOptions(
		{{{"j", "json"},
		  "setting file describing labelig positions, distances, etc",
		  "file"},
		 {"pdb", "PDB file", "path"},
	...
	if (pdbPath.isEmpty() && dirPath.isEmpty()) {
		std::cerr
			<< "Neither --pdb nor --dir where specified. Quitting."
			<< std::endl;
		return 3;
	}
	...
	QJsonDocument doc =
		QJsonDocument::fromJson(jsonFile.readAll(), &jsonErr);
	if (doc.isNull()) {
		std::cerr << "Invalid json file" << std::endl;
		std::cerr << jsonErr.errorString().toStdString() + " ("
				     + std::to_string(jsonErr.offset) + ")"
			  << std::endl;
		return 2;
	}
	...
	if (!pdbPath.isEmpty()) {
		auto pdbFnamePtr = std::make_shared<const std::string>(
			pdbPath.toStdString());
