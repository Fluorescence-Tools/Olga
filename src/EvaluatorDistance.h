...
public:
	EvaluatorDistance(const TaskStorage &storage, const EvalId &av1,
	...
	virtual Setting setting(int row) const override
	{
		...
		case 0:
			return {"distance_type",
				QString::fromStdString(_dist.type())};
		case 1: {
			EvalId id =
	...
	virtual void setSetting(int row, const QVariant &val)
	{
		...
		case 1: {
			...
			if (tmpId == EvalId(-1)) {
                std::cerr << "Can not set position1 for " + name()
						     + "\n"
					  << std::flush;
				return;
			}
		...
		case 2: {
			auto tmpId = val.value<EvalId>();
			if (tmpId == EvalId(-1)) {
                std::cerr << "Can not set position2 for " + name()
						     + "\n"
					  << std::flush;
				return;
			}
