	...
	void save(const std::string fileName)
	{
		...
		for (const auto &vec : {coords, velocities}) {
			for (int i = 0; i < coords.size(); ++i) {
				const auto &v = vec[i];
				for (int k : {0, 1, 2}) {
					f << setiosflags(ios::fixed) << setw(12)
					  << setprecision(7) << v[k];
				}
				if (i % 2 == 1) {
					f << "\n";
...
struct NmrRestraint {
	...
	std::string toString() const
	{
		std::string str = "#" + note + "\n";
		using std::to_string;
		str += "&rst iat = " + to_string(_iat1) + ", "
		       + to_string(_iat2) + ", nstep1 = " + to_string(_nstep1)
		       + ", nstep2 = " + to_string(_nstep2) + ", ifvari = 1"
		       + ", r1 = " + to_string(r1) + ", r2 = " + to_string(r2)
		       + ", r3 = " + to_string(r3) + ", r4 = " + to_string(r4)
		       + ", rk2 = " + to_string(rk2) + ", rk3 = "
		       + to_string(rk3) + ", r1a = " + to_string(r1a)
		       + ", r2a = " + to_string(r2a) + ", r3a = "
		       + to_string(r3a) + ", r4a = " + to_string(r4a)
		       + ", rk2a = " + to_string(rk2a)
		       + ", rk3a = " + to_string(rk3a) + ",\n/\n";
		return str;
	}
...
int main(int argc, char *argv[])
{
	...
	if (numDUAtoms == 0) {
		...
		atom.mass = 100.0f;
		atom.charge = 0.0f;
		auto resids = sys.select(std::string("all")).get_resid(true);
